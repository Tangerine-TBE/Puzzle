...
import com.weilai.library.utils.FragmentChangeManager;
import com.weilai.library.utils.UnreadMsgUtils;
import com.weilai.library.widget.MsgView;
...
public class FlyTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            if (mIconGravity == Gravity.LEFT) {
                ...
                tabView = View.inflate(mContext, R.layout.layout_tab_bottom_coustom, null);
            }
...
//        Log.d("AAA", "mCurrentP--->" + mCurrentP.left + "&" + mCurrentP.right);
    ...
    private void calcIndicatorRect() {
        ...
        if (mIndicatorWidth < 0) {   //indicatorWidth小于0时,原jpardogo's PagerSlidingTabStrip
            ...
            float indicatorLeft = currentTabView.getLeft() + (currentTabView.getWidth() - mIndicatorWidth - dp2px(8)) / 2;
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        } else {//indicatorWidth大于0时,圆角矩形以及三角形,这里减8dp 是因为layout_tab_bottom_coustom 的ll_tap布局有paddingRight="8dp"
            float indicatorLeft = p.left + (currentTabView.getWidth() - mIndicatorWidth - dp2px(8)) / 2;
...
//setter and getter
    ...
    public void showMsg(int position, int num) {
        ...
        if (tipView != null) {
           ...
           /* if(num <=0){
            ...
            if (!mIconVisible && mIconGravity != Gravity.TOP) {
    ...
    private FlyTabLayout.IndicatorPoint mCurrentP = new FlyTabLayout.IndicatorPoint();
    private FlyTabLayout.IndicatorPoint mLastP = new FlyTabLayout.IndicatorPoint();

    class PointEvaluator implements TypeEvaluator<FlyTabLayout.IndicatorPoint> {
        @Override
        public FlyTabLayout.IndicatorPoint evaluate(float fraction, FlyTabLayout.IndicatorPoint startValue, FlyTabLayout.IndicatorPoint endValue) {
            float left = startValue.left + fraction * (endValue.left - startValue.left);
            float right = startValue.right + fraction * (endValue.right - startValue.right);
            FlyTabLayout.IndicatorPoint point = new FlyTabLayout.IndicatorPoint();
            point.left = left;
            point.right = right;
            return point;
        }
